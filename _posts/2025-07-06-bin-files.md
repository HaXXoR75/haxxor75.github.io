...
modified_date: 2026-02-27 12:55:00 -0300
last_modified_at: 2026-02-27 12:55:00 -0300
---
...
## DECADE LUNA - 2014 {#decade-luna}
Early versions of Decade relied on heavy protection of the client EXE to protect their key, via Enigma Protector, before ditching that in later versions, likely for performance reasons, and moving to a more sophisticated BIN format instead around 2016. They were the first server I know of to use an alteration of the actual BIN header as an obfuscation. All later Decade LUNA Bin files have an extra DWORD in the header, between the version and type fields, that in all known examples has the value of `20052007`. This number doesn't seem to be used in any actual calculations by Decade LUNA, but rather serves to significantly break any standard BIN editor trying to parse the header. They were also one of the first to use a non-date based version for their BIN files, along with [Arch LUNA](#arch-luna).
```cpp
...
```
```cpp
20131029, "AZ4ZELB1NF1L3S777113"	//older bins, normal header
42400598, "B14L2317246E56H627V3"	//newer bins, decade extended header
```
...
## ROYAL LUNA - 2017 {#royal-luna}
Another interesting case, while they have their own key, they used Eya's version for it, whether on purpose or by simply not understanding the purpose of the version field, is unclear. Interestingly, this means that their client can't read any BINs encrypted with EyaSoft's normal keys, they had to re-encrypt every single BIN file themselves with their key.
```cpp
20101117, "915H3825287T19G082K8" //Conflicts with Eya's 20101117 key
```

## DESTINY LUNA - 2018 {#destiny-luna}
Uses the exact same setup as [Royal LUNA](#royal-luna) above, implying this was just a re-spin of the same server under a new name.
